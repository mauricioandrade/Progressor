import { tile } from '../styles/shared';
    ...
    return (
        <div className={tile}>
            <div className="flex items-center justify-between mb-3">
                ...
                <button aria-label="Editar meta de água" onClick={() => { setEditingGoal(v => !v); setGoalInput(water.dailyWaterGoal > 0 ? String(water.dailyWaterGoal) : ''); }} className="w-6 h-6 flex items-center justify-center rounded-full bg-black/5 dark:bg-white/10 text-gray-400 hover:text-blue-400 transition-colors">
            ...
            {editingGoal ? (
                <div className="flex gap-1.5 items-center mb-3">
                    ...
                    <button aria-label="Salvar meta de água" onClick={submitGoal} className="w-7 h-7 flex items-center justify-center rounded-xl bg-blue-500 text-white"><Check className="w-3.5 h-3.5" aria-hidden="true" /></button>
                    <button aria-label="Cancelar" onClick={() => setEditingGoal(false)} className="w-7 h-7 flex items-center justify-center rounded-xl bg-black/10 dark:bg-white/10 text-gray-400"><X className="w-3.5 h-3.5" aria-hidden="true" /></button>
                </div>
            ) : (
                <>
...
function PendingInvitesCard() {
    ...
    const { data: invites = [] } = usePendingConnections();
    ...
    async function respond(requestId: string, accepted: boolean) {
        ...
        try {
            ...
            queryClient.setQueryData(QK.pendingConnections, (prev: ConnectionInvite[]) =>
                (prev ?? []).filter(i => i.id !== requestId));
...
function StudentDashboard() {
    const { t } = useTranslation();
    const queryClient = useQueryClient();
    const { data: exercises = [], isPending: loadingExercises } = useMyWorkouts();
    const { data: measurements = [], isPending: loadingMeasurements } = useMyMeasurements();
    const { data: prs = [], isPending: loadingPRs } = useMyPRs();
    const { data: checkInDates = [], isPending: loadingCheckIns } = useMyCheckIns();
    const { data: mealPlan = null, isPending: loadingMealPlan } = useMyMealPlan();
    const { data: water = { dailyWaterGoal: 0, currentWaterIntake: 0 }, isPending: loadingWater } = useMyWater();
    const { data: weightGoal = null, isPending: loadingGoal } = useMyWeightGoal();

    const isLoading = loadingExercises || loadingMeasurements || loadingPRs || loadingCheckIns || loadingMealPlan || loadingWater || loadingGoal;

    const todayCode = getTodayCode();
    const todayExercises = exercises.filter(e => e.scheduledDays?.includes(todayCode));
    ...
    async function handleAddWater(amount: number) {
        try {
            ...
            queryClient.setQueryData(QK.myWater, r.data);
    ...
    async function handleSaveGoal() {
        ...
        try {
            ...
            queryClient.setQueryData(QK.myWeightGoal, val);
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Weight goal progress tile — always visible for students */}
            <div className={tile}>
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                    ...
                    {!editingGoal && (
                        <button aria-label="Editar meta de peso" onClick={() => { setEditingGoal(true); setGoalInput(weightGoal?.toString() ?? ''); }} className="w-6 h-6 flex items-center justify-center rounded-full bg-black/5 dark:bg-white/10 text-gray-400 hover:text-blue-500 transition-colors">
                            <Pencil className="w-3 h-3" aria-hidden="true" />
                        </button>
                    )}
                </div>
                {editingGoal ? (
                    <div className="flex gap-1.5 items-center mb-3">
                        <input type="number" min={1} step={0.1} value={goalInput} onChange={e => setGoalInput(e.target.value)} placeholder={t('weight_goal.placeholder')} className="flex-1 px-2.5 py-1.5 border border-black/10 dark:border-white/20 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 text-xs focus:ring-2 focus:ring-blue-500 outline-none" onKeyDown={e => { if (e.key === 'Enter') handleSaveGoal(); }} />
                        <button aria-label="Salvar meta de peso" onClick={handleSaveGoal} disabled={isSavingGoal} className="w-7 h-7 flex items-center justify-center rounded-xl bg-blue-500 text-white disabled:opacity-50"><Check className="w-3.5 h-3.5" aria-hidden="true" /></button>
                        <button aria-label="Cancelar" onClick={() => setEditingGoal(false)} className="w-7 h-7 flex items-center justify-center rounded-xl bg-black/10 dark:bg-white/10 text-gray-400"><X className="w-3.5 h-3.5" aria-hidden="true" /></button>
                    </div>
                ) : weightGoal === null ? (
                    <div className="py-4 flex flex-col items-center justify-center text-center gap-2">
                        <p className="text-sm font-semibold text-blue-500">{t('weight_goal.no_goal')}</p>
                        <p className="text-xs text-gray-400">{t('weight_goal.set_goal_hint')}</p>
                    </div>
                ) : (
                    <div className="flex items-end justify-between mb-2">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-lg font-bold text-blue-500">{weightGoal} kg</p>
                        </div>
                    </div>
                )}
                {!editingGoal && weightGoal !== null && latestWeight !== null && (
                    <>
                ...
                {config.links.map(({ to, labelKey, iconColor, hoverBorder, hoverIcon, Icon }) => (
                    <Link key={to} to={to} className={`${tile} flex items-center justify-between group ${hoverBorder} !p-4`}>
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-medium text-gray-700 dark:text-gray-200">{t(labelKey)}</span>
                        </div>
                        <ChevronRight className={`w-4 h-4 text-gray-400 ${hoverIcon} transition-colors`} />
                    </Link>
                ))}
            </div>
        </div>
...
export function Dashboard() {
    ...
    const { data: profile = null } = useMyProfile();
    const { data: avatarUrl = null } = useMyAvatar();
        ...
        <div className="flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-slate-950 dark:to-slate-900">
            ...
            <div className="flex-1 min-w-0">
                ...
                <main className="p-4 md:p-6 max-w-3xl pb-24 md:pb-6">
                    ...
                    {user.role === 'PERSONALTRAINER' && <ProfessionalDashboard role="PERSONALTRAINER" />}
                    {user.role === 'NUTRITIONIST' && <ProfessionalDashboard role="NUTRITIONIST" />}
                </main>
            </div>
        </div>
    );
